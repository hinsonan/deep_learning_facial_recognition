...
if __name__ == '__main__':
    ...
    dirs = os.listdir('data/two_stage')
    actual = []
    predicted = []
    for dir in dirs:
        for file in os.listdir(f'data/two_stage/{dir}'):
            img = Image.open(f'data/two_stage/{dir}/{file}')
    ...
    for x,y in zip(actual,predicted):
        if x != y:
            count += 1
    print((len(actual) - count) / len(actual) )
